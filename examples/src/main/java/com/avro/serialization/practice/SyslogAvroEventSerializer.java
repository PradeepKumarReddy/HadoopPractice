...
import com.avro.serialization.practice.SyslogAvroEventSerializer.SyslogEvent;
...
public class SyslogAvroEventSerializer extends AbstractAvroEventSerializer<SyslogEvent> {
	private static final DateTimeFormatter dateFmt1 = DateTimeFormat
			.forPattern("MMM dd HH:mm:ss").withZoneUTC();
	private static final DateTimeFormatter dateFmt2 = DateTimeFormat
	...
	// But since this is a test class, having the schema in an .avsc file is
	// more
	// readable. Should probably just use the maven avro plugin to generate
	// the inner SyslogEvent class from this file.
	private static final File schemaFile = new File(
			"src/test/resources/syslog_event.avsc");
	...
	protected SyslogEvent convert(Event event) {
		...
		if (headers.containsKey(SyslogUtils.SYSLOG_FACILITY)) {
			...
			int facility = Integer.parseInt(headers
					.get(SyslogUtils.SYSLOG_FACILITY));
			sle.setFacility(facility);
		}
		if (headers.containsKey(SyslogUtils.SYSLOG_SEVERITY)) {
			fromSyslogSource = true;
			int severity = Integer.parseInt(headers
					.get(SyslogUtils.SYSLOG_SEVERITY));
	...
	/**
	 * Returns epoch time in millis, or 0 if the string cannot be parsed. We use
	 * two date formats because the date spec in rfc3164 is kind of weird. <br/>
	...
	private static long parseRfc3164Date(String in) {
		...
		try {
			...
			logger.debug("Date parse failed on ({}), trying single-digit date",
					in);
		}
		if (date == null) {
			try {
				...
				// ignore the exception, we act based on nullity of date object
				logger.debug(
						"2nd date parse failed on ({}), unknown date format",
						in);
			}
		}
	...
	public static class Builder implements EventSerializer.Builder {
		public EventSerializer build(Context context, OutputStream out) {
			...
			try {
				...
				logger.error("Unable to parse schema file. Exception follows.",
						e);
			}
			return writer;
		}
